...
### Changed
- Refined `filephile-icon-ios.svg` to a full-bleed opaque border plate (edge-to-edge cyan gradient border) with a stronger inner cyan edge glow, so iOS/Android apply their own squircle mask over an already-opaque tile — light/dark mode can never show through the corners. All `icons/icon-*.png`, `icons/icon-512-maskable.png`, `icons/apple-touch-icon.png`, and `favicon.ico` were re-rendered from the updated master via `scripts/generate-icons.mjs`
...
- Icon cache-busters bumped to `?v=1.10` across `index.html`, `manifest.webmanifest`, `sw.js`, and `browserconfig.xml`; Service Worker cache bumped to `filephile-v1.13` so installed clients pick up the new icon set. Note: iOS caches the Home Screen icon per install — remove and re-add the web clip to see the change
